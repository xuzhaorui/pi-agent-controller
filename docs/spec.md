...
Pi 在长期会话、会话内模型切换和 token 利用率方面很适合作为持续的软件工程工作台；WSL 则提供真实的 Git、测试、构建和 benchmark 环境。现有子代理扩展已经能把编码、测试等任务委派给隔离的 Worker，但一次委派完成后，主 Agent 往往停止，无法可靠地继续领取下一个 GitHub Issue。
...
用户在 Pi 中启动一个 Run。Controller 根据 Policy 从 GitHub 发现带有 `ready-for-agent` 标签的 Task，确定性地领取其中一个 Task，在独立 Git Worktree 中创建 Workspace，通过可替换的 Agent Runtime 启动 Worker，要求 Worker 返回经过 schema 校验的 Result，然后执行项目配置的 Verification。通过验证后，Controller 启动 Reviewer；审查通过后按 Policy 合并、同步 GitHub Issue 并继续领取下一项 Task。
...
第一版强调安全、可恢复和可解释，而不是追求完全自治：单个 Controller 实例、单个活动 Task、有限重试、有限 Run Budget、显式启动与恢复、默认失败关闭。Worker 与 Reviewer 使用隔离的 Pi 子进程；初始 Agent Runtime 遵循 Pi 官方子代理示例的 JSON 模式与取消传播方式，同时保留适配现有 pi-open-agents 的边界。
...
5. As a Tech Lead, I want to stop a Run immediately when necessary, so that I can contain unsafe or wasteful behavior.
...
25. As a Worker Agent, I want a handoff containing the Task goal, acceptance criteria, constraints, architecture context, Workspace, and verification expectations, so that I can execute without guessing intent.
26. As a Worker Agent, I want to return a structured Result, so that the Controller can process completion without parsing conversational prose.
27. As a Controller, I want to schema-validate every Result, so that malformed Agent output cannot advance the state machine.
28. As a Controller, I want to capture changed files, commit identity, verification claims, risks, blockers, and usage from each Execution, so that subsequent decisions have evidence.
29. As a Controller, I want a Worker process failure, timeout, cancellation, or malformed Result to become an explicit failed attempt, so that failure is never mistaken for completion.
...
36. As a Reviewer Agent, I want the Task, diff, architecture constraints, and Verification Evidence, so that I can assess correctness, scope, safety, and maintainability.
...
55. As a Tech Lead, I want Controller events and Agent outputs to redact known secrets before persistence and display, so that auditability does not leak credentials.
...
62. As a Pi Package user, I want session shutdown and reload to cancel or checkpoint Controller resources cleanly, so that no orphan Agent process remains.
...
64. As a maintainer, I want GitHub, Agent Runtime, Git Workspace, Verification, and persistence behind ports, so that external dependencies can be tested and replaced without changing the state machine.
...
69. As a maintainer, I want abort signals propagated from Pi through Controller to child Agent and Verification processes, so that user cancellation is effective.
...
- The architecture separates a deterministic Controller Core from adapters for Issue Tracker, Git Workspace, Agent Runtime, Verification, persistence, clock, and UI. The Core does not call GitHub, Git, shell, or Pi APIs directly.
- The primary domain vocabulary is: Project, Backlog, Task, Run, Reconcile Loop, Policy, Workspace, Agent Role, Execution, Handoff, Result, Verification, Review, Evidence, Human Gate, Blocker, Run Budget, Run Journal, State Transition, and Stop Reason.
...
- Workspace creation, claim updates, comments, Agent starts, Verification commands, and merges use idempotency keys recorded before or with the side effect. Recovery reconciles journal intent with actual Git and GitHub state.
- Agent Runtime is a port. The initial adapter starts isolated Pi subprocesses in JSON mode, selects the configured model and tools by Agent Role, streams bounded progress, records usage, and propagates cancellation, following Pi’s official subagent extension pattern.
- Existing pi-open-agents compatibility is achieved through the Agent Runtime boundary rather than coupling Controller Core to another extension’s internal implementation. A direct adapter may be added when a stable callable contract is available.
...
- Every Agent must finish through a terminating structured Result contract. Conversational “done” text is insufficient. Result parsing is strict; invalid output becomes a failed attempt.
...
- Recovery scenarios restart the Core from a persisted Journal while adapters report actual GitHub and Git state. Tests verify idempotency after crashes before and after claims, Workspace creation, Agent completion, comments, merge, and Issue closure.
...
- Agent Runtime behavior uses a deterministic fake at the Reconcile seam for success, structured failure, malformed Result, timeout, cancellation, token usage, and requested changes. A thin subprocess contract test verifies Pi JSON event parsing, model/tool arguments, output termination, usage collection, and abort propagation.
...
- The first release should optimize the complete sequential happy path and its recovery path before adding more Agent roles or parallelism.
